						   ...
						   <#elseif po.showType=='file' || po.showType == 'image'>
							...
							<#elseif po.showType=='tree'>
								<input id="${po.fieldName}" ${po.extendJson?if_exists}  type="text"
							       style="width: 150px" class="form-control" <@datatype showType="2" validType="${po.fieldValidType!''}" isNull="${po.isNull}" type="${po.type}" mustInput="${po.fieldMustInput!''}" isNull="${po.isNull}"/>
								...
								<script>
									...
									function show${po.fieldName?cap_first }Tree(){
										 $.ajax({  
									        ...
									        dataType:'JSON',
									        async:false,
									        <#-- update-begin-author:taoyan date:20180326 for:树控件根据字典code过滤  -->
									        data:{
								        		selfCode:'${po.dictField}'
								        	},
									        <#-- update-end-author:taoyan date:20180326 for:树控件根据字典code过滤  --> 
									        success:function(res){
									            ...
									            $("#show${po.fieldName?cap_first }TreeContent").slideDown("fast");  
									...
									function show${po.fieldName?cap_first }Tree(){
										 $.ajax({  
									        ...
									        success:function(res){
									            ...
									            var deptObj = $("#${po.fieldName}");  
									            var deptOffset = $("#${po.fieldName}").offset();  
									            <#-- update-begin-author:taoYan date:20180323 for:TASK #2574 【代码生成器】代码生成效果有问题 1. 树控件样式  -->
									            $("#show${po.fieldName?cap_first }TreeContent").css({top:(deptObj[0].offsetTop + deptObj.outerHeight()) + "px"}).slideDown("fast");  
									            <#-- update-end-author:taoYan date:20180323 for:TASK #2574 【代码生成器】代码生成效果有问题 1. 树控件样式  -->
									            $('#show${po.fieldName?cap_first }Tree').css({width:deptObj.outerWidth() - 12 + "px"});  
									            var zTree = $.fn.zTree.getZTreeObj("show${po.fieldName?cap_first }Tree");  
									            var idVal =  $("#${po.fieldName}Id").val();
									            if(idVal != null && idVal != ''){
