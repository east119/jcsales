						...
						<#-- update--begin--author:zhangjiaqiang date:20170815 for:TASK #2274 【online】Online 表单支持树控件 -->
							<#elseif po.show_type=='tree'>
								<input id="${po.field_name}" ${po.extend_json?if_exists}${po.show_type}  type="text"
							       <#-- update-begin-author:taoYan date:20170826 for:树控件input样式bug -->
							       class="form-control" 
							       <#-- update-end-author:taoYan date:20170826 for:树控件input样式bug -->
									<#if po.field_must_input??><#if po.field_must_input == 'Y' || po.is_null != 'Y'>ignore="checked"<#else>ignore="ignore"</#if><#elseif po.is_null != 'Y'> ignore="checked"<#else>ignore="ignore"</#if>
					                 <#if po.field_valid_type?if_exists?html != ''>
						               datatype="${po.field_valid_type?if_exists?html}"
						               <#else>
										 ...
										 if($("#show${po.field_name?cap_first }TreeContent").is(":hidden")){
										 	 $.ajax({  
										        ...
										        success:function(res){
										            ...
										            <#--update-begin-Author:taoYan  Date:20180323 for:ace风格因为input父元素浮动导致下拉树top定位不准  -->
										            $("#show${po.field_name?cap_first }TreeContent").css({top:(deptObj[0].offsetTop + deptObj.outerHeight()) + "px"}).slideDown("fast");  
										            <#--update-end-Author:taoYan  Date:20180323 for:ace风格因为input父元素浮动导致下拉树top定位不准  -->
